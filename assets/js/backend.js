...
$(document).on('click','.genrateMeetingId',function (){
    ...
    $("#meeting_number").css("border", "1px solid floralwhite;color")
  
   toastr.success('Meeting Id Create - '+x);
});
    ...
    $("#mockPaperStartDate").datepicker({
        dateFormat: "dd-mm-yy",
        changeMonth: true,
        changeYear: true,
        onSelect: function(date) {
            var minDate = $(this).datepicker('getDate');
            $('.edu_accordion_container .mockPaperStartDate').datepicker('option', 'minDate', minDate);
            $('#b_endDate').datepicker('option', 'minDate', minDate);
        }
    });
	...
	toastr.error('You are not allowed to delete');
        	return false;
        swal({
                title: ltr_are_you_so_msg,
    ...
    $(document).on('click', '.addChapers, .editChapterName', function() {
        if ($(this).hasClass('editChapterName')) {
            // 	toastr.error('You are not allowed to Edit');
        // 	return false;
    ...
    $(document).on('click', '.createFinalExamPaper', function() {
        var validchk = validate_form($(this).closest('form'));
        // if (GetTodayDate() >=  $('.mockPaperStartDate').val()) {
        //      toastr.error(`${$('.mockPaperStartDate').val()} is less than ${GetTodayDate()}`);
        //     return false;
        // }
        // if (formatAMPM(new Date) >= $('.chooseTime').val()) {
        //   toastr.error(`${$('.chooseTime').val()} is less than ${formatAMPM(new Date)}`);
        //   return false;
        // } 
  ...
  $(document).on('click', '.addGalleryPop, .editVideoImgGalry', function() {
        ...
        if ($(this).hasClass('addGalleryPop')) {
            ...
            parentDiv.find('form').trigger('reset');
            parentDiv.find('.addVideoImgGalry').val('add');
            parentDiv.find('#gallery_id').val('');
             parentDiv.find('.fileNameShow').html('');
...
//         var parentDiv = $('#add_image_video');
//         var pxn_amin = $('.pxn_amin');
//         var pxn_amin = $('.pxn_amin');
      
//         if ($(this).hasClass('addGalleryPop')) {
//                         pxn_amin.trigger('reset');
//             parentDiv.find('#myModalLabel1').html(ltr_add_image);
//             parentDiv.find('.addVideoImgGalry').attr('data-id', 'add');
//             parentDiv.find('#gallery_id').val('');
//         } else {
//             parentDiv.find('#myModalLabel1').html(ltr_edit_image);
//             parentDiv.find('.addVideoImgGalry').val('Edit');
...
//     });
    ...
    $(document).on('click', '.submitPopupShow', function() {
        ...
        //alert(dt);
        var n = dt.toLocaleString([], { hour: '2-digit', minute: '2-digit' ,second :'2-digit'});
        $('.question_paper_footer #submit_time').val(startTime());
    ...
    $(document).on('click', '.SubmitPaperForm', function() {
        ...
        $('#submitPopup').find('.mfp-close').trigger('click');
        var stime = $('#start_time').attr('stime');
    ...
    $(document).on('click','.restartPepar',function refreshPage(){
        window.location.reload();
    });
    ...
    $(document).on('click', '.addjetsiClass', function() {
       var validchk = validate_form($(this).closest('form'));
        
        if (validchk == 'valid') {
            var type = $(this).attr('data-type');
            if (type == 'add') {
              var url = 'ajaxcall/add_live_class_jetsi';
            }else {
              var url = 'ajaxcall/edit_live_class_jetsi';
            }
            var formdata = new FormData($(this).closest('form')[0]);
            $('.edu_preloader').css('background-color', 'rgba(255,255,255,0.80)').css('display', 'block');
            $.ajax({
    ...
    $(document).on('click', '.jetsi_add_live_class', function() {
        var parentDiv = $('#JetsiclassSettingModal');
        parentDiv.find('#jetsi_add_live_class').val($(this).attr('data-id'));

        $.magnificPopup.open({
            items: {
                src: '#JetsiclassSettingModal',
            },
            type: 'inline'
        });
    });
    ...
    $('.startjetsiMeet').on('click', function() {
        var formdata = new FormData($(this).closest('form')[0]);
        var valid_check = validate_form($(this).closest('form'));
        if (valid_check == 'valid') {
            $.ajax({
                method: "POST",
                url: base_url + 'ajaxcall/JetsiMeeting',
                data: formdata,
                processData: false,
                contentType: false,
                success: function(resp) {
 ...
 $('.term_condition').each(function(e) {
    ...
    $(document).on('click', '.addAdminPop, .edit_admin', function() {
        ...
        if ($(this).hasClass('addAdminPop')) {
            ...
            // 		$('#input_feilds_admin').find('[name="primary_color"]').val('#4d4a81');
...
$(document).on('click', '.edit_teacher,.edit_admin ', function() {
    ...
    $.ajax({
        ...
        success: function(resp) {
            var resp = $.parseJSON(resp);
            var access = $.parseJSON(resp.data.access);
            console.log(access.assignment);
            $('#input_feilds_teacher').find('[name="name"]').val(resp.data.name).trigger('change');
            $('#input_feilds_teacher').find('[name="email"]').val(resp.data.email).trigger('change');
            $('#input_feilds_teacher').find('[name="teach_education"]').val(resp.data.teach_education).trigger('change');
            $('#input_feilds_teacher').find('[name="teach_gender"]').val(resp.data.teach_gender).trigger('change');
            $('#input_feilds_teacher').find('[name="password"]').removeClass('require').closest('.form-group').find('label sup').addClass('hide');
            $('#input_feilds_teacher').find('.fileNameShow').html(resp.data.teach_image);
            if (access.academics == 1) {
                $('.academics').prop('checked', 'checked')
            } else {
                $('.academics').prop('checked', false)}
            ...
            if (access.gallary_manage == 1) {
                $('.gallary_manage').prop('checked', 'checked')
            } else { $('.gallary_manage').prop('checked', false) }
            if (access.library_manager == 1) {
                $('.library_manager').prop('checked', 'checked')
            } else { $('.library_manager').prop('checked', false) }
            if (access.teacher_manager == 1) {
                $('.teacher_manager').prop('checked', 'checked')
            } else { $('.teacher_manager').prop('checked', false) }
...
$(document).on('click', '.themesColorAdd', function() {
       var formdata = new FormData($(this).closest('form')[0]);
        $.ajax({
            method: "POST",
            url: base_url + "ajaxcall/themesOption",
            data: formdata,
            processData: false,
            contentType: false,
            success: function(resp) {
...
$('.quetion_hide_show').hide();
$('.create_question_new').hide();
  
 $(document).on('click','#addQuestion',function(){
    var chk = $(this).val();
    // alert(chk);
    if($(this).is(":checked")) {
        $(".quetion_hide_show").show(300);
        $('#addQuestion').val('1');
    } else {
        $(".quetion_hide_show").hide(200);
        $('#addQuestion').val('');
    }
});
$(document).on('click','#CreateQuestion',function(){
    ...
    // alert(chk);
    if($(this).is(":checked")) {
        $(".create_question_new").show(300);
        $('#CreateQuestion').val('1');
    } else {
        $(".create_question_new").hide(200);
        $('#CreateQuestion').val('1');
    }
});
$(".marking_value").hide();
$(document).on('click','#add_marking',function(){
    var chk = $(this).val();
    // alert(chk);
    if($(this).is(":checked")) {
        $(".marking_value").show(300);
        $(".markingNegative").addClass('require');
    } else {
        $(".marking_value").hide(200);
         $(".markingNegative").removeClass('require');
    }
});
$(document).on('click','#add_marking_d',function(){
    var chk = $(this).val();
    // alert(chk);
    if($(this).is(":checked")) {
        $(".marking_value").hide(200);
...
$('.paper_update_edit').hide();    
$(document).on('click', '.edit_paper_manage', function() {
    var id = $(this).attr('data-id'); 
    $('.PaperTable').hide();
    $.ajax({
        method: "POST",
        url: base_url + "ajaxcall/paper_manage_edit",
        data: { id: id },
        success: function(resp) {
            $('.paper_update_edit').show();
            var resp = $.parseJSON(resp);
            // console.log(resp);
            $('.edit_manage_paper').find('form').trigger('reset');
              $('.edit_manage_paper').find('[name="type"]').val('').trigger('change');
                $('.edit_manage_paper').find('[name="batch_id"]').val('').trigger('change');
                  $('input[name="name"]').val('');
                    $('input[name="mock_sheduled_date"]').val('');
                     $('input[name="mock_sheduled_time"]').val('');
                       $('input[name="time_duration"]').val('');
                         $('input[name="mock_sheduled_date"]').val('');
                          $('input[name="mock_sheduled_date"]').val('');
            setTimeout(function() {
                $('.edit_manage_paper').find('[name="name"]').val(resp['exam_paper'][0]['name']);
                $('.edit_manage_paper').find('[name="expid"]').val(resp['exam_paper'][0]['id']);
                $('.edit_manage_paper').find('[name="type"]').val(resp['exam_paper'][0]['type']).trigger('change');
                $('.edit_manage_paper').find('[name="mock_sheduled_date"]').val(resp['exam_paper'][0]['mock_sheduled_date']);
                $('.edit_manage_paper').find('[name="mock_sheduled_time"]').val(resp['exam_paper'][0]['mock_sheduled_time']);
                $('.edit_manage_paper').find('[name="time_duration"]').val(resp['exam_paper'][0]['time_duration']);
                $('.edit_manage_paper').find('[name="batch_id"]').val(resp['exam_paper'][0]['batch_id']).trigger('change');
                $('.edit_manage_paper').find('[name="mock_sheduled_date"]').val(resp['exam_paper'][0]['mock_sheduled_date']);
                $('.edit_manage_paper').find('[name="mock_sheduled_date"]').val(resp['exam_paper'][0]['mock_sheduled_date']);
                var i =1;
                $.each(resp['question_data'], function(key,val) {
                   var ht_q  = '<li class="question_m paper_update_e" style="list-style: none;"><i class="fa fa-times remove_button"></i><input hidden type="text" name="old_question_id[]" value="'+val[0]['id']+'"><div class="edu_single_questionView_wrap "><div class="edu_singleView_question questionsDiv"><div class="quest">Q'+i+++'  '+val[0]['question']+'</div><br></div></div></li>';
                //   var html_q = '<div class="col-lg-6 col-md-12 col-sm-12 col-12"><div class="form-group"><input type="text"id="time_durarion" value="'+val['question']+'" class="form-control require" name="question_old"></div></div>';
                      $('.q_data').append(ht_q);
                    //   console.log(val);
                });
            }, 200);
...
$(document).on('click', '.Update_paper', function() {
    var value = CKEDITOR.instances['question'].getData();
    var option1 = CKEDITOR.instances['option1'].getData();
    var option2 = CKEDITOR.instances['option2'].getData();
    var option3 = CKEDITOR.instances['option3'].getData();
    ...
    var formdata = new FormData($(this).closest('form')[0]);
        ...
        formdata.append('option3', option3);
        formdata.append('option4', option4);
        // var ans= $('.ansRadioChck').val();
        // alert(ans);
        // return false;
        if ($('#addQuestion').val()=="1"){
            if($('#old_queston_p').val()==""){
                toastr.error('Please Selecet Any Question');
                return false;
            }
        }
        if($('#CreateQuestion').val()=="1"){
            if($('.modalSubjectCls').val()==""){
                toastr.error('Please Select Subject ');
                return false;
            }else if($('.filter_modal_chapter ').val()==""){
                toastr.error('Please Select Chapter  ');
                return false;
            }else if(value==""){
                toastr.error('Question Are Blank ');
                return false;
            }else if(option1==""){
                toastr.error('Option one Is Blank ');
                return false;
            }else if(option2==""){
                toastr.error('Option Two Is Blank ');
                return false;
            }else if(option3==""){
                toastr.error('Option Three Is Blank ');
                return false;
            }else if(option4==""){
                 toastr.error('Option Four Is Blank ');
                return false;
            }else if($('.ansRadioChck').val()==""){
                 toastr.error('Please Select Any Answer');
                return false;
            } 
        }
        $.ajax({
            method: "POST",
            url: base_url + "ajaxcall/update_paper",
            data:formdata ,
            processData: false,
            contentType: false,
            success: function(resp) {
    ...
    $(document).on('click', '.loginSubAdmin', function() {
       var subAdminId = $(this).attr('data-id'); //32 admin id 
      $.ajax({
            method: "POST",
            url: base_url + "front_ajax/login",
...
$(document).on('click','.batchesGet',function(){
    var sid = $(this).attr('sis');
    $('.charactersViewResultBatch').empty();
    $.ajax({
        method: "POST",
        url: base_url + "ajaxcall/getBatches",
        data: {sid:sid},
        success: function(resp) {
            var resp = $.parseJSON(resp);
            console.log(resp.data.length==0);
            if(resp.data.length==0){
                 toastr.error('No Batch Purchased');
            }else{
                $.each(resp.data, function (index, value) {
                  $('.charactersViewResultBatch').append('<a class="tbl_status_btn edu_btn_width light_sky_bg" href="javascript:;">'+value.batch_name+'</a>&nbsp;&nbsp;'); 
                });
...
$(document).on('click', '.add_mask', function() {
    var formdata = new FormData();
    var t = $(this).attr('data-table');
    var c = $(this).attr('data-column');
    var add_mask = $('#add_mask').val();
    AllIdArray = [];
    $('.tableFullWrapper').find('.checkOneRow:checked').each(function() {
        var value = $(this).val();
        if (AllIdArray.indexOf(value) === -1) AllIdArray.push(value);
    ...
    formdata.append('ids', JSON.stringify(AllIdArray));
    formdata.append('table_name', t);
    formdata.append('column', c);
    formdata.append('mask', add_mask);
    ...
    $.ajax({
        method: "POST",
        url: base_url + "ajaxcall/add_mask",
        data: formdata,
        processData: false,
        contentType: false,
        success: function(resp) {
...
$(".academics").on('click', function() {
  ...
  $(document).on('change', '#template_id', function() {
      ...
      $('.optionCheck').show();
       $('.optionCheck').empty();
        if ($(this).val() == '1') {
            $('.optionCheck').append('<img src="'+base_url+'assets/images/certificate/default.png" class="default" width="300" height="200" >');
        } else if ($(this).val() == '2'){
            $('.optionCheck').append('<img src="'+base_url+'assets/images/certificate/template_one.png" class="default" width="300" height="200" >');
        }else{
            $('.optionCheck').append('<img src="'+base_url+'assets/images/certificate/template_two.png" class="default" width="300" height="200" >');
        }
    });