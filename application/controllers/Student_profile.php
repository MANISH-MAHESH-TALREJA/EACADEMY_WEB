...
// 		die;
	...
	function homework(){
	   ...
	   //   print_r($_SESSION);die;
		...
		$data['homeworks_data'] = $this->db_model->select_data('*','homeworks  use index (id)',array('batch_id'=>$this->session->userdata('batch_id'),'homeworks.date >= '=>date('Y-m-d')),'',array('id','desc'));

		$this->load->view("common/student_header",$header);
		$this->load->view("student/homework",$data); 
		$this->load->view("common/student_footer");
    }
    public function JetsiMeeting($mid){
      
	    $livedata =$this->db_model->select_data('*','jetsi_setting',array('batch' =>$mid));
	    ...
	    if(empty($livedata)){
    		...
    		$this->load->view("student/meeting",$data);
	    }
	}
...
// 		$data['paper_list'] = $this->db_model->select_data('id,name,time_duration,total_question','exams use index (id)',array('type'=>2,'admin_id'=>$this->session->userdata('admin_id'),'status'=>1,'batch_id'=>$this->session->userdata('batch_id')),'',array('id','desc'));
		...
		$data['practice_data'] = $this->db_model->select_data('','exams  use index (id)',array('admin_id'=>$this->session->userdata('admin_id'),'type'=>2),'',array('id','desc'));
...
// 		$data['mock_data'] = $this->db_model->select_data('id','exams  use index (id)',array('admin_id'=>$this->session->userdata('admin_id'),'batch_id'=>$this->session->userdata('batch_id'),'type'=>1),'',array('id','desc'));
				$data['paper_list'] = $this->db_model->select_data('id,name,time_duration,total_question,mock_sheduled_date,mock_sheduled_time','exams use index (id)',array('type'=>1,'status'=>1,'batch_id'=>$this->session->userdata('batch_id'),'mock_sheduled_date >='=>date('Y-m-d')),'',array('id','desc'));
		$data['mock_data'] = $this->db_model->select_data('id','exams  use index (id)',array('batch_id'=>$this->session->userdata('batch_id'),'type'=>1),'',array('id','desc'));
	...
	function student_attendance(){
		...
		$data['attendance'] = $this->db_model->select_data('id','attendance',array('student_id'=>$_SESSION['uid']),1);
...
// 		$batch_id = $this->session->userdata('batch_id');
	...
	//	$this->session->userdata('batch_id')
		...
		$data['book_data'] = $this->db_model->select_data('*','book_pdf  use index (id)',array('admin_id'=>$this->session->userdata('admin_id')),'',array('id','desc'),$like);
	    $this->load->view("common/student_header",$header);
