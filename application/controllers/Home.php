...
$data['getsubjectchapter'] = $this->db_model->select_data('*','batch_subjects use index (id)',array('batch_id'=>$id),'','','','');
	...
	function paypal_success(){ 
	  ...
	  $data['title']=$this->lang->line('ltr_home_page');
	  $data['frontend_details'] = $this->db_model->select_data('*','frontend_details',array('id'=>'1'),1);
	  $this->load->view('common/front_header',$data);
      $this->load->view('frontend/success');
      $this->load->view('common/front_footer', $data);
    } 
	function themes_option(){
      $data['title'] = $this->lang->line('ltr_themes_option');
      $data['frontend_details'] = $this->db_model->select_data('*','frontend_details',array('id'=>'1'),1);
      $data['courses'] = $this->db_model->select_data('course_name','courses use index (id)',array('status'=>'1','admin_id'=>'1'),5);
      $data['facilities'] = $this->db_model->select_data('title','facilities use index (id)',array('status'=>'1'),5);
      $data['terms'] = $this->db_model->select_data('*','term_condition_data',array('id'=>'1'),1);
      $data['theme_color'] = $this->db_model->select_data('*','theme_color',array('id'=>'1'),1);
      $this->load->view('common/admin_header',$data);
      $this->load->view('admin/themes_option',$data);
...
// 		die();
    ...
    public function testMail(){
         $this->SendMail('manishtalreja189@gmail.com','test','test');
    }
    ...
    function test(){
        ...
        curl_setopt($ch, CURLOPT_URL, 'https://bigbluebutton.org/bigbluebutton/api/create');
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_POSTFIELDS, "welcome=Welcome&allowStartStopRecording=true&attendeePW=ap&autoStartRecording=false&meetingID=random-1730297&moderatorPW=mp&name=random-1730297&record=false&voiceBridge=71296&checksum=1234;");
    ...
    function ccavenue(){
    	...
    	$currency     = 'INR';
    	$mid          = 2545815;
    	$redirect_url = 'http://www.jobpe.co/home/ccav_resp';
    	$cancel_url   = 'http://www.jobpe.co/home/ccav_resp';
    
    	$access_code = 'AVHR75KF98AS43RHSA';	//Shared by CCAVENUES
    	$working_key = 'DB4AB9404A358560BF177064F5ECAC2F';	//Shared by CCAVENUES
    
    	$merchant_data = 'currency='.$currency.'&merchant_id='.$mid.'&order_id='.$order_id.'&redirect_url='.$redirect_url.'&cancel_url='.$cancel_url.'&amount='.$amount;
    	$data['encrypted_data'] = encrypt($merchant_data, $working_key); // Method for encrypting the data.
		$data['access_code'] = $access_code;
    	$this->load->view('ccavenue',$data);
    }    
    public function ccav_resp(){
        if(isset($_POST['encResp'])){
            $this->load->helper('common');
            $workingKey     = "DB4AB9404A358560BF177064F5ECAC2F";	//Working Key should be provided here.
            $encResponse    = $_POST['encResp'];					//This is the response sent by the CCAvenue Server
            $rcvdString     = decrypt($encResponse, $workingKey);	//Crypto Decryption used as per the specified working key.
            $this->Common_model->insert('test_ccav',array('ccav_resp'=>$rcvdString));
            echo "<pre>".$rcvdString; //decrypted response
        }else{
              redirect(base_url('home/ccavenue'));
        }
    }
