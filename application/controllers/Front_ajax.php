...
class Front_ajax extends CI_Controller {
 ...
 function login(){
                 ...
                 $captcha_response = trim($this->input->post('g-recaptcha-response'));
                    ...
                    if(empty($prevRecd)){
                        ...
                        if(empty($prevRecd_mobile)){
                            ...
                            $data_arr['google_token']= $captcha_response;
                            // print_r($data_arr);
                            // die();
                            ...
                            if($ins){
                                ...
                                $title = $this->db_model->select_data('site_title','site_details','',1,array('id','desc'))[0]['site_title'];
                                    ...
                                    $em_msg = $this->lang->line('ltr_hey').' '.ucwords($name).', '.$this->lang->line('ltr_congratulations').' <br/><br/>'.$this->lang->line('ltr_successfully_enrolled').'<br/><br/>'.$this->lang->line('ltr_login_details').'<br/><br/> '.$this->lang->line('ltr_enrolment_id').' : '.$enrolid.'<br/><br/>'.$this->lang->line('ltr_password').' : '.$password.'<br/><br/>'.$this->lang->line('ltr_url').' : <br/><br/>'.$this->lang->line('ltr_thnx_msg').'';
                                    $em_msg = 'Your UserName is <b>'. $enrolid. '</b> and password is ' .$password;
    ...
    function email_template(){
        ...
        $this->load->view('frontend/email_template');
    }
    
    //  function sendLoginEmail($to='', $subject = '', $body = ''){
    //     $tos[] = array(
    //     'email' => $to,
    //     'name' => '',
    //     'type' => 'to'
    ...
    //     'html' => $body,	
    //     'subject' => $subject,
    //     'from_email' => $this->general_settings('smtp_mail'),
    //     'from_name' => 'E Academy',
    //     'to' =>$tos,
    //     ); 	   
    //     $POSTFIELDS = array(
...
function change_student_status(){
    ...
    function student_signUp($name='',$email='',$mobile='',$batchId=''){
            ...
            $batches_aid = $this->db_model->select_data('id,admin_id','batches use index (id)',array('id'=>$batchId),1)[0]; 
			$admin_id = $batches_aid['admin_id'];
			$prevRecd = $this->db_model->select_data('multi_batch,id as studentId,email as userEmail,name as fullName,enrollment_id as enrollmentId,contact_no as mobile,app_version as versionCode, batch_id as batchId,admin_id as adminId,admission_date as admissionDate, image, token,admin_id','students use index (id)',array('email'=>$email),1);
			...
			if(empty($prevRecd)){
				...
				$tempBid = json_decode($prevRecd[0]['batchId']);
                array_push($tempBid, $batch_type[0]['id']);
                $multi_batch_id = json_encode($tempBid);
                ...
                // die;
				$data_arr['name'] = $name;
				$data_arr['batch_id'] = $multi_batch_id;
				$data_arr['added_by'] = 'student';
				$data_arr['status'] = 1;
				$data_arr['contact_no']=$prevRecd[0]['mobile'];
				$data_arr['login_status']= 0;
    ...
    function homewrok_table(){
        ...
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            ...
            $cond = array('homeworks.date >= '=>date('Y-m-d'),'homeworks.batch_id'=>$this->session->userdata('batch_id'));
    ...
    function get_paper_details(){
        ...
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            if(!empty($this->input->post('id',false))){
               ...
               if($paperDetails[0]['marking_parcent']==""){
                    $per = "0.25";
                }else{
                    $per = $paperDetails[0]['marking_parcent'];
                }
                if(!empty($paperDetails)){
                    if($this->input->post('paper_type',TRUE) == 'practice'){
                        $instructions = '<li>'.$this->lang->line('ltr_all_questions_com').'</li>
                            <li>'.$this->lang->line('ltr_there_will_be_one').$paperDetails[0]['marking_parcent'].' %'.$this->lang->line('ltr_there_will_be_two').'</li>
                        ...
                        <div class="question_paper_inner">
                                ...
                                <div class="row">
                                    <div class="col-lg-6 col-md-6 col-sm-12 col-12 ">
                                        <div class="edu_paper_info">
                                            ...
                                            <p><span class="edu_paper_smallinfo">Total Marks : </span>'.$paperDetails[0]['total_marks'].'</p>
                                        </div>
                                    </div>
                                    <div class="col-lg-6 col-md-6 col-sm-12 col-12">
                                            <div class="edu_paper_info">
                            ...
                            <div class="question_paper_body">
                                <div class="edu_question_section">
                                    ...
                                    $questionData = $this->db_model->select_data('id,question,options,answer,question_mask','questions use index (id)',"id in ($question_id)",'',array('id','desc'));
                                                ...
                                                $html .= '<li>
                                                    <div class="edu_single_question_wrap questionDesrpWrap" data-id="'.$ques['id'].'" data-ans="'.$ques['answer'].'">
                                                        <div class="edu_single_question">
                                                            ...
                                                            <span class="Question_marks">'.$ques['question_mask'].' Marks</span>
                                                            <div class="edu_rest_btn resetAnswer"><a href="javascript:void(0)"><i class="icofont-ui-reply"></i></a>
                                                            </div>
                                                        </div>
                            ...
                            <div class="question_paper_footer">
                                <div class="edu_btn_wrapper">
                                    <form method="post">
                                        ...
                                        <input type="text" hidden name="" stime="'.$stime.'" id="start_time" value="'.date('H:i:s').'">
...
function submit_paper(){
        ...
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            ...
            if(!empty($this->input->post('question_answer',TRUE))){
                $data_arr = $this->input->post(NULL,TRUE);
                if(count(json_decode($data_arr['question_answer'],true))==0){
                    $resp = array('status'=>'5', 'msg' => $this->lang->line('ltr_paper_attempt'));
                }else{
                    ...
                    foreach($attemptedQuestion as $key=>$value){
                        if(isset($right_ansrs[$c])){
                            if(($key == $right_ansrs[$c]['id']) && ($value == $right_ansrs[$c]['answer'])){
                                        $rightCount1++;
                                         array_push($wrt_id,$right_ansrs[$c]['id']);
                                    }else{
                                        $wrongCount1++;
                                       array_push( $wrong_id,$right_ansrs[$c]['id']);
                                    }
                                }
                                $c++;
                     }
                    foreach($wrong_id as $key=>$value){
                         $wrong_ansrs = $this->db_model->select_data('question_mask', 'questions use index (id)',array('id'=>$value));
                        $wrongCount += $wrong_ansrs[0]['question_mask'];
                    }
                    foreach($wrt_id as $key=>$value){
                         $wrong_ansrs = $this->db_model->select_data('question_mask', 'questions use index (id)',array('id'=>$value));
                        $rightCount += $wrong_ansrs[0]['question_mask'];
                    }
                    
                    $negative_marking = $this->db_model->select_data('marking_parcent,id,total_marks','exams  use index (id)',array('id'=>$this->input->post('paper_id',TRUE)));
                
                    if($negative_marking[0]['marking_parcent']==0){
                     $total_score = ($rightCount )/$negative_marking[0]['total_marks'];
                    }else if($negative_marking[0]['marking_parcent']!=0){
                     $per = $wrongCount*$negative_marking[0]['marking_parcent'];
                     $total_score = ($rightCount - $per)/$negative_marking[0]['total_marks'];
                    }
   ...
   function result_table($type) {
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            ...
            if($type == 'practice'){
               ...
               $table_name = 'mock_result';
               $cond1=array('type'=>1,'concat(mock_sheduled_date ," ",mock_sheduled_time)<=' => date('Y-m-d H:i:s'),'batch_id'=>$this->session->userdata('batch_id'));
            //   $cond1=array("admin_id"=>$this->session->userdata('admin_id'),'type'=>1,'concat(mock_sheduled_date ," ",mock_sheduled_time)<=' => date('Y-m-d H:i:s'),'batch_id'=>$this->session->userdata('batch_id'));
            }
