...
class Ajaxcall extends CI_Controller{ 
    ...
    //                         $config['max_height'] = 430;
                        ...
                        if($ins==true){
							...
							if(!empty($data['batch_speci_heading'])){
								...
								// 		'batch_fecherd' => '['.json_encode($data['batch_sub_fecherd'][$i]).']',
								// 	); 
								$speci_heading = array(
        								'batch_id'	=> $batch_id,
        								'batch_specification_heading' => $data['batch_speci_heading'][$i],
                        ...
                        if(!empty($chapterdata)){
                            ...
                            if($role != 1){
                                $action = '<p class="actions_wrap">
                                    <a href="javascript:;" data-tcsid="'.$sub['subject_id'].'" data-tcbid="'.$sub['batch_id'].'" class="btn_view tc_progress_popup" title="View progress"><i class="fa fa-eye"></i></a>
                                    <a href="'.base_url().'teacher/student-details/'.$sub['batch_id'].'" data-tcsid="'.$sub['subject_id'].'" data-tcbid="'.$sub['batch_id'].'" class="btn_view student_details" title="View progress"><i class="fa fa-users" aria-hidden="true" title="Student Details"></i></a>
                                    <a class="edit_batchDetails btn_edit" data-id="'.$sub['id'].'" data-index="'.$dataIndx.'" data-batchon="'.$batchon.'"><i class="fa fa-edit" title="Edit"></i></a>
                                </p>';
                            }else{              
    ...
    //             $or_like = array(array('multi_batch' ,$this->session->userdata('uid')));
    //         }
    ...
    //                 $cond = ($cond4.' in ('.$batch_ids.') AND '.$cond1.'.'.$cond5.'= 1'); 
    //     		}else{
    //     			$cond = '';
    //     		}
    //         }
            
    //         if(isset($get['user_status']) || isset($get['lgStatus'])){
    //             if($get['user_status']!='' && $get['lgStatus']!=''){
    //                 $cond = array('admin_id'=>$uid,'status'=>$get['user_status'],'login_status'=>$get['lgStatus']);
    //                   $or_like = "";
    //             }else  if($get['user_status']!='' && $get['lgStatus']==''){
    //                 $cond = array('admin_id'=>$uid,'status'=>$get['user_status']);
    //                   $or_like = "";
    //             }else if($get['lgStatus']!='' && $get['user_status']==''){
    //               $cond = array('admin_id'=>$uid,'login_status'=>$get['lgStatus']);
    //                  $or_like = "";
    //             }
    //         }
    ...
    function student_Details(){
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            ...
            if($post['search']['value'] != ''){
                ...
                $like = ''; 
                $or_like = array(array('batch_id' ,$id));
            }
            if($get['user_batch']!=''){
            ...
            $cond='';
            $student_dat = $this->db_model->select_data('students.admin_id as aid,students.id as s_id,students.payment_status as paymentStatus,students.status,students.admission_date,students.name,students.email,students.contact_no,students.enrollment_id','students',$cond,$limit,array('students.id','asc'),$like,'','',$or_like,$wherein);
        ...
        //   echo $this->db->last_query();die;
            ...
            if(!empty($student_dat)){
                ...
                foreach($student_dat as $student){
                 ...
                 $recordsTotal = $this->db_model->countAll('students use',$cond,'','',$like,'','',$or_like);
    ...
    function getBatches(){
        ...
        if(empty($batch)){
            ...
            foreach($batch as $btch){
                $batch_array = $this->db_model->select_data('id,batch_name','batches use index (id)',array('id'=>$btch['batch_id']))[0];
                array_push($bt,$batch_array);
            }
        }
        echo json_encode(array('status'=>1,'data'=>$bt),JSON_UNESCAPED_SLASHES);
    }
            ...
            if(!empty($this->input->post('email',TRUE))){
                ...
                if($this->input->post('type',TRUE) == 'edit'){
                   ...
                   $batchName = array();
                    ...
                    $data1['name'] = $data_arr['name'];
                    $data1['email'] = $data_arr['email'];
    			    $data1['password'] = '';
    			    $data1['link'] ='';
    			    $data1['status'] ='Update';
        ...
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            ...
            $subject_data = $this->db_model->select_data('*','subjects use index (id)',$cond,$limit,array('id','desc'),$like,'','',$or_like);
             if(!empty($subject_data)){
                ...
                if($_SESSION['admin_id']!=$subject['admin_id']){
                    $action = '<p class="actions_wrap"><a class="btn_edit edit_SbjectChaptr" title="Edit" data-id="'.$subject['id'].'"><i class="fa fa-edit"></i></a>
...
function subcategory_table(){
            ...
            if(!empty($question_data)){
                ...
                if($_SESSION['admin_id']!=$question['added_by']){
                   ...
                   if($vid['status'] == 1){
                    ...
                    if($page == 'exam'){
                        $dataarray[] = array(
                            // '<input type="checkbox" class="checkOneRow" value="'.$question['id'].'"><input type="text" clas="ques_marks" name="question_mask[]" hidden value="'.$question['question_mask'].'">',
        ...
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            ...
            if(!empty($setting_data)){
                ...
                if($type =="zoom"){
                    ...
                    $recordsTotal = $this->db_model->countAll('jetsi_setting',array('jetsi_setting.admin_id' => $admin_id),$like,'','',array('batches','batches.id=jetsi_setting.batch'),'','',$where_in);
                }
                $output = array(
                    "draw" => $post['draw'],
                    "recordsTotal" => $recordsTotal,
    ...
    function liveclasscheckstudent($id){
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
    		...
    		$livedata_his =$this->db_model->select_data('*','live_class_history',array('end_time'=>''),'',array('id','desc'));
    
            if(!empty($livedata_his)){
              
                foreach($livedata_his as $lcs){
                	$batchName =$this->db_model->select_data('*','batches',array('id'=>$lcs['batch_id']),'',array('id','desc'));
                    if($lcs['type_class'] ==1){
                        $action = '<p class="actions_wrap"><a class="btn_view class_join_student" title="Live Class" data-id="'.$setting['id'].'"><i class="fa fa-users" aria-hidden="true"></i></a></p>';
                    }else{
                        $action = '<p class="actions_wrap"><a class="btn_view class_join_student" title="Live Class" data-id="'.$setting['id'].'"><i class="fa fa-users" aria-hidden="true"></i></a></p>';
                    }
                    $dataarray[] = array(
                ...
                $recordsTotal = $this->db_model->countAll('live_class_history',array('end_time' => ''),'','','','','','','');
                $output = array(
                    "draw" => $post['draw'],
                    "recordsTotal" => $recordsTotal,
                    "recordsFiltered" => $recordsTotal,
  ...
  function exam_table(){
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            ...
            if($this->session->userdata('role')==1 &&  $this->session->userdata('super_admin')==1){
                ...
                if(!empty($batch_id)){
                //$cond = "admin_id = $admin_id AND batch_id in ($batch_id)";
                $cond = array('admin_id'=> $this->session->userdata('admin_id'),'batch_id'=> $this->session->userdata('batch_id'));
                }
                $btchcond = "admin_id = $admin_id AND id in ($batch_id)";
            }
            // print_r($cond);die;
            // die();
            $all_exams = $this->db_model->select_data('*','exams use index (id)',$cond,$limit,array('id','desc'),$like,'','');
            $super_a_id = $this->session->userdata('super_admin');
            if(!empty($all_exams)){
                $batch_array = $this->db_model->select_data('id,batch_name','batches use index (id)',$btchcond);
                ...
                foreach($all_exams as $exam){
                    ...
                    if($this->session->userdata('super_admin')==1 && $this->session->userdata('role')==1){
                       if($this->session->userdata('uid')==$exam['admin_id'] && $this->session->userdata('role')==1){
                            $action = '<p class="actions_wrap">
                            <a class="edit_paper_manage  ml-2 " title="Edit" data-id="'.$exam['id'].'"><i class="fa fa-edit" ></i></a>
                            <a href="'.base_url('admin/view-paper/'.$exam['id']).'" target="_blank" class="btn_view" title="View"><i class="fa fa-eye"></i></a>
                            <a class="deleteData btn_delete" title="Delete" data-id="'.$exam['id'].'" data-table="exams"><i class="fa fa-trash"></i></a></p>';
                       }else{
                            $action = '<p class="actions_wrap"><a class=" ml-2" title="Edit disabled" data-id="'.$exam['id'].'"><i class="fa fa-edit disabled" ></i></a><a href="'.base_url('admin/view-paper/'.$exam['id']).'" target="_blank" class="btn_view" title="View"><i class="fa fa-eye"></i></a>
                            <a class="btn_delete disabled" title="Delete" data-id="'.$exam['id'].'" data-table="exams"><i class="fa fa-trash disabled"></i></a></p>';
                       }
                        ...
                        if($exam['added_by'] == $this->session->userdata('uid') && $this->session->userdata('role')==3){
                            $action = '<p class="actions_wrap">
                            <a class="edit_paper_manage  ml-2 " title="Edit" data-id="'.$exam['id'].'"><i class="fa fa-edit" ></i></a>
                            <a href="'.base_url('teacher/view-paper/'.$exam['id']).'" target="_blank" class="btn_view" title="View"><i class="fa fa-eye"></i></a>
                            <a class="deleteData btn_delete" title="Delete" data-id="'.$exam['id'].'" data-table="exams"><i class="fa fa-trash"></i></a>
                            </p>';
                            if($exam['format'] == 1){
                            $format ="Shuffle";
                            }else{
    ...
    function add_exam_paper(){
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            if(!empty($this->input->post('name',TRUE))){
                ...
                $question_marks = [];
                foreach(json_decode($_POST['question_ids']) as $value){
                    $prevData_question = $this->db_model->select_data('id,question_mask','questions use index (id)',array('id'=>$value),1);
                    array_push($question_marks,$prevData_question[0]['question_mask']);
                }
                foreach ($question_marks as $key=>$value){
                    $data_arr1['total_marks'] +=$question_marks[$key];
                     
                }
                $data_arr1['total_question'] = $data_arr['total_question'];
                $data_arr1['type'] = $data_arr['type'];
                $data_arr1['name'] = $data_arr['name'];
                $data_arr1['time_duration'] = $data_arr['time_duration'];
                $data_arr1['batch_id'] = $data_arr['batch_id'];
                $data_arr1['marking_parcent'] = $data_arr['marking_parcent'];
                $data_arr1['question_ids'] = $data_arr['question_ids'];
                
                if($this->session->userdata('role')==1)
                    $data_arr1['admin_id'] = $this->session->userdata('uid');
                else
                    $data_arr1['admin_id'] = $this->session->userdata('admin_id');

                if($this->input->post('type',TRUE) == 1){
                    $data_arr1['mock_sheduled_date'] = date('Y-m-d',strtotime($data_arr['mock_sheduled_date']));
                    $data_arr1['mock_sheduled_time'] = date('H:i:s',strtotime($data_arr['mock_sheduled_time']));
                    ...
                    $data_arr1['mock_sheduled_date'] = '';
                    $data_arr1['mock_sheduled_time'] = '';
                    $prevData = array();
                }
                $ngmks = isset($_POST['negative_marking'])?$_POST['negative_marking'] : '';
                if($ngmks=='on'){
                    $data_arr1['marking_parcent'] = "0";
                }else if($_POST['negative_marking']==1 && $_POST['marking_parcent']!=""){
                    $data_arr1['marking_parcent'] =  $_POST['marking_parcent'];
                }else{
                    $data_arr1['marking_parcent'] ="0.25";
                }
            
                $data_arr1['status'] = 1;
                $data_arr1['format'] = 1;
                $data_arr1['added_at'] = date('Y-m-d H:i:s');
                $data_arr1['added_by'] = $this->session->userdata('uid');
                if($this->session->userdata('role') == 1){
                    $profile = 'admin';
                }else{
                    $profile = 'teacher';
                ...
                if(empty($prevData)){
                    ...
                    $ins = $this->db_model->insert_data('exams',$data_arr1);               
                    if($ins==true){
...
function result_table($type){
        ...
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            if(!empty($this->input->post('title',TRUE))){
               ...
               if($this->input->post('upload',TRUE) == 'File'){
                ...
                if($ins==true){
                    $tp = strtolower($this->input->post('type',TRUE));
                    $resp = array('status'=>1,'msg'=>$this->lang->line('ltr_gallery').' '.$tp.' '.$this->lang->line('ltr_updated_msg'));
                }else{
                    $resp = array('status'=>0);
                }
            }else{
        ...
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            if(!empty($this->input->post('id',TRUE))){
                ...
                if($this->input->post('table',TRUE) == 'students'){
                    ...
                    $res = $this->db_model->delete_data('sudent_batchs',array('student_id'=>$this->input->post('id',TRUE)));
                }
         ...
         if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            ...
            if($this->session->userdata('role') == 1 && $super == 1){ 
               $cond = array('student_id !='=>'0', 'leave_management.admin_id' => $this->session->userdata('uid'));
                ...
                $cond = array('student_id !='=>'0', 'leave_management.admin_id' => $this->session->userdata('admin_id'));
            }else{
               $cond = array('student_id !='=>'0', 'leave_management.admin_id' => $this->session->userdata('uid'));
            }
    ...
    function add_live_class_jetsi(){
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            if(!empty($this->input->post('batch',TRUE))){
                $setting_data = $this->db_model->select_data('*','jetsi_setting',array('batch'=>$this->input->post('batch',TRUE)));
                if(empty($setting_data)){
                    $data_arr['batch'] = $this->input->post('batch',TRUE);
                    $data_arr['meeting_number'] = $this->input->post('meeting_number',TRUE);
                    $data_arr['password'] = $this->input->post('password',TRUE);
                    ...
                    $data_arr['admin_id'] = $this->session->userdata('uid');
                    $data_arr = $this->security->xss_clean($data_arr);
                    $ins = $this->db_model->insert_data('jetsi_setting',$data_arr);
                    if($ins==true){
                        $resp = array('status'=>1,'msg'=>$this->lang->line('ltr_class_added_msg'));
                    }else{
                        $resp = array('status'=>0);
    ...
    function edit_live_class_jetsi(){
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            if(!empty($this->input->post('batch',TRUE))){
                $setting_data = $this->db_model->select_data('*','jetsi_setting',array('batch'=>$this->input->post('batch',TRUE),'id !='=>$this->input->post('live_class_id',TRUE)));
                if(empty($setting_data)){
                    ...
                    $data_arr['password'] = $this->input->post('password',TRUE);
                    $data_arr = $this->security->xss_clean($data_arr);
                    $ins = $this->db_model->update_data_limit('jetsi_setting',$data_arr,array('id'=>$this->input->post('live_class_id',TRUE)),1);
                    if($ins==true){
                        $resp = array('status'=>1,'msg'=>$this->lang->line('ltr_class_added_msg'));
                    }else{
                        $resp = array('status'=>0);
    ...
    function live_class_jetsi_table(){
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            $post = $this->input->post(NULL,TRUE);
            $get = $this->input->get(NULL,TRUE);
            if(isset($post['length']) && $post['length']>0){
            ...
            if($post['search']['value'] != ''){
                $like = array('jetsi_setting.meeting_number',$post['search']['value']);
            }else{
               $like = ''; 
            }
            if(isset($get['admin'])){
                if($get['admin']!=''){   
                    $like = array('jetsi_setting.admin_id',$get['admin']); 
                }
            }
            if($this->session->userdata('role') == 1 && $this->session->userdata('super_admin') == 1){  
                $cond = array('jetsi_setting.admin_id'=>$this->session->userdata('uid'));
            //   $cond = '';
            }else{
               $cond = array('jetsi_setting.admin_id'=>$this->session->userdata('uid'));
            }
            ...
            $setting_data = $this->db_model->select_data('jetsi_setting.*,batches.batch_name','jetsi_setting',$cond,$limit,array('jetsi_setting.id','desc'),$like,array('batches','batches.id=jetsi_setting.batch'));
            ...
            if(!empty($setting_data)){
                ...
                foreach($setting_data as $setting){
                    $action = '<p class="actions_wrap"><a class="edit_live_class btn_edit" data-table-name="jetsi_setting" title="Edit" data-id="'.$setting['id'].'" data-batch="'.$setting['batch'].'"><i class="fa fa-edit"></i></a>
                        <a class="deleteData btn_delete" title="Delete" data-id="'.$setting['id'].'" data-table="jetsi_setting"><i class="fa fa-trash"></i></a>
                        <a class="btn_view add_live_class_admin" title="Live Class" data-id="'.$setting['id'].'" data-batch="'.$setting['batch'].'"><i class="fa fa-users" aria-hidden="true" ></i></a>
                ...
                $recordsTotal = $this->db_model->countAll('jetsi_setting',$cond,'','',$like);
                ...
                $name = $this->db_model->select_data('*','users use index (id)',array('id'=>$setting['admin_id']),1);
                 ...
                 if($name){
                    ...
                    $dataarray[] = array(
                                ...
                                $setting['type_class']==1?'Zoom':'Jitsi',
                                $added_by
    ...
    function student_attendance($id,$month,$year){
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            ...
            if($_SESSION['role']==1){
                $cond = array('student_id'=> $id,'admin_id'=>$_SESSION['uid']);
                $like = array('date',$year.'-'.$month);
            }else if($_SESSION['role']==3){
                $cond = array('student_id'=> $id,'added_id'=>$_SESSION['uid']);
                $like = array('date',$year.'-'.$month);
            }else{
                $cond = array('student_id'=> $id,'batch_id'=>$_SESSION['batch_id']);
                $like = array('date',$year.'-'.$month);
            }
    ...
    function readMoreWord($story_desc, $title='',$C_word='') {
        ...
        if($count_word>$chars){
            $readMore = '<a class="charaViewPopupModel tbl_status_btn light_sky_bg" data-title="'.$title.'" data-word="'.$story_desc.'"  href="javascript:;"> More </a>';
	...
	function student_doubts_class($tid) {
	  if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            ...
            if($post['search']['value'] != ''){
                ...
                $like = array('name',$post['search']['value']);
                $or_like = '';
				$userdata = $this->db_model->select_data('id,name','students',array('student_doubts_class',"student_doubts_class.student_id = students.id"),'',array('id','desc'),$like);
				$usersId =array();
				...
				// $condd = "student_id in ($uId) AND teacher_id = $tid]";
					$condd = array('admin_id'=>$_SESSION['uid'],'teacher_id'=>$tid);
            }else{
               $like = ''; 
               $or_like = ''; 
            }
            ...
            if($this->session->userdata('role')==1){
                if(!empty($get['subject'])){
                    $cond = array('admin_id'=>$_SESSION['uid'],'subjects_id'=>$get['subject']);
                }else{
        		...
        		//print_r($doubts_data);die;
//             if(!empty($condd)){
// 				if(!empty($uId)){
// 					$doubts_data = $this->db_model->select_data('*','student_doubts_class',array('admin_id'=>$_SESSION['uid'],'teacher_id'=>$tid),$limit,array('doubt_id','desc'));
// 				}else{
// 					$doubts_data = '';
// 				}
// 			}else{
        ...
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            ...
            if(!empty($doubts_data)){
                ...
                foreach($doubts_data as $key=>$value){
                    ...
                    $userName =$this->db_model->select_data('name,image,id','students',array('id'=>$value['student_id']),1);
                  if(!empty($userName)){
                    $batchName = $this->db_model->select_data('batch_name,start_date,end_date','batches',array('id'=>$value['batch_id']),1);
         ...
         function add_book(){
            ...
            if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
                ...
                if(!empty($id)){
                   ...
                   $batch_data = $data_arr['batch_id'];
                //   $data_arr2['batch']=json_encode($data_arr['batch_id']);
                   $data_arr2['batch']=$data_arr['batch_id'];
                   $data_arr2['subject']= $data_arr['subject_id'];                  
                   $data_arr2['status'] = 1; 
                   $data_arr2['added_by'] = $this->session->userdata('uid');
                   $data_arr2['added_at'] = date('Y-m-d H:i:s'); 
    ...
    function book_table(){
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            ...
            $get = $this->input->get(NULL,TRUE);
             $role = $this->session->userdata('role');
             $super = $this->session->userdata('super_admin');
            $student_Btach = json_decode($_SESSION['batch_id']);
            ...
            if($post['search']['value']!=''){
                $like =array('title',$post['search']['value']); 
                $cond = array('admin_id'=>$this->session->userdata('admin_id'),'batch'=>$_SESSION['batch_id']);
                $or_like = ''; 
            }else{
                $or_like = ''; 
                if(isset($get['subject'])){
                     $like = array('batch',$_SESSION['batch_id']); 
            ...
            if(!empty($videos)){
                foreach($videos as $vid){
                    if($_SESSION['admin_id']!=$vid['added_by']){
                        ...
                        <a class='deleteData btn_delete' title='Delete' data-id='".$vid["id"]."' data-table='book_pdf'><i class='fa fa-trash'></i></a></p>";
                         ...
                         $action = " 
                            ...
                            <p class='actions_wrap'>
                            <a class='btn_edit' title='Edit'><i class='fa fa-edit disabled'></i></a>
                            <a class='viewPdf_ btn_view' title='View' data-id='".$vid["id"]."' href='".base_url("admin/file-view/book/").$vid["id"]."' target='_blank' data-url='uploads/book/".$vid["file_name"]."'><i class='fa fa-eye'></i></a>
                   ...
                   if($_SESSION['admin_id']!=$vid['added_by']){
                    ...
                    // $bach_name = '';
                      $bach_name = current($this->db_model->select_data('batch_name','batches use index (id)',array('id'=>$vid['batch']),1))['batch_name'];
    ...
    function add_notes(){
        ...
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            if(!empty($this->input->post('title',TRUE))){
                ...
                $data_arr['batch']=$data_arr['batch'][0];
                $data_arr['added_by'] = $this->session->userdata('uid');
                $data_arr['status'] = 1; 
                $data_arr['added_at'] = date('Y-m-d H:i:s'); 
                 if(isset($_FILES['pdf_file']) && !empty($_FILES['pdf_file']['name'])){
     ...
     function notes_table(){
        ...
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            ...
            if($role == 1 && $super == 1){  
                ...
                $cond = array('status'=>1,'batch'=>$_SESSION['batch_id']);
            }
        
            $notes_pdf = $this->db_model->select_data('*','notes_pdf use index (id)',$cond,$limit,array('id','desc'),$like,'','',$or_like);
            // echo $this->db->last_query();
            if(!empty($notes_pdf)){
                
                foreach($notes_pdf as $vid){ 
                    if($_SESSION['admin_id']!=$vid['added_by']){
                        $action = "
            ...
            if(!empty($videos)){
                ...
                foreach($videos as $vid){
                 ...
                 if($_SESSION['admin_id']!=$vid['added_by']){
                    ...
                    <p class='actions_wrap'><a class='btn_edit' title='Edit'><i class='fa fa-edit disabled'></i></a><a class='viewPdf_ btn_view' title='View' data-id='".$vid["id"]."' href='".base_url("admin/file-view/old_paper/").$vid["id"]."' target='_blank' data-url='uploads/notes/".$vid["file_name"]."'><i class='fa fa-eye'></i></a>
...
function themesOption(){
       if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            ...
            // die;
            $admin_themes = array(
                'admin_primary' => $data_arr['admin_primary'],
                'admin_secondary' => $data_arr['admin_secondary'],
                'admin_accent' => $data_arr['admin_accent'],
                'admin_text' => $data_arr['admin_text'],
                'admin_alternate' => $data_arr['admin_alternate'],
                'admin_header' => $data_arr['admin_header'],
              );
            $teacher_themes = array(
                'teacher_primary' => $data_arr['teacher_primary'],
                'teacher_secondary' => $data_arr['teacher_secondary'],
                'teacher_accent' => $data_arr['teacher_accent'],
                'teacher_text' => $data_arr['teacher_text'],
                'teacher_alternate' => $data_arr['teacher_alternate'],
                'teacher_header' => $data_arr['teacher_header'],
              );
            $student_themes = array(
                'student_primary' => $data_arr['student_primary'],
                'student_secondary' => $data_arr['student_secondary'],
                'student_accent' => $data_arr['student_accent'],
                'student_text' => $data_arr['student_text'],
                'student_alternate' => $data_arr['student_alternate'],
                'student_header' => $data_arr['student_header'],
              );
            $frontend_themes = array(
                'frontend_primary' => $data_arr['frontend_primaryColor'],
                'frontend_secondary' => $data_arr['frontend_togglePaletteOnly1'],
                'frontend_accent' => $data_arr['frontend_accentColor'],
                'frontend_text' => $data_arr['frontend_textColor'],
                'frontend_alternate' => $data_arr['frontend_alternateTextColor'],
                'frontend_header' => $data_arr['frontend_BgColorid'],
              );
            $login_themes = array(
                'login_primary' => $data_arr['login_primary'],
                'login_secondary' => $data_arr['login_secondary'],
                'login_alternate' => $data_arr['login_accentColor'],
              );
            $themesColorAll = array(
                'admin_id'=>$_SESSION['uid'],
                'admin_themes'=>json_encode($admin_themes),
                'teacher_themes'=>json_encode($teacher_themes),
                'student_themes'=>json_encode($student_themes),
                'frontend_themes'=>json_encode($frontend_themes),
                'login_themes'=>json_encode($login_themes),
                );
            //  print_r($themesColorAll);
            //  die;
            if(!empty($data_arr['themesid'])){
                 $ins = $this->db_model->update_data_limit('theme_color',$themesColorAll,array('id'=>$data_arr['themesid']),1);
                if($ins){
                    $resp = array('status'=>'1', 'msg' =>$this->common->languageTranslator('ltr_update_themes_color'));
                }else{
                    $resp = array('status'=>'0', 'msg' =>$this->common->languageTranslator('ltr_add_themes_color_error'));
                }
            }else{
                $ins = $this->db_model->insert_data('theme_color',$themesColorAll); 
                if($ins){
                    $resp = array('status'=>'1', 'msg' =>$this->common->languageTranslator('ltr_add_themes_color'));
                }else{
                    $resp = array('status'=>'0', 'msg' =>$this->common->languageTranslator('ltr_add_themes_color_error'));
                }
            }
 ...
 /*multi admin code start */    
        ...
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            ...
            if(!empty($teachers)){
                foreach($teachers as $teach){
                    ...
                    $action = '<p class="actions_wrap">
                                <a class="edit_admin btn_edit" data-id="'.$teach['id'].'" data-img="'.$teach['teach_image'].'" ><i class="fa fa-edit""></i></a>
                                <a class="deleteData btn_delete" data-id="'.$teach['id'].'" data-table="users"><i class="fa fa-trash"></i></a>
                            </p>';
                    $sa = '<p class="actions_wrap">
                               <a  class="loginSubAdmin actions_wrap" data-id="'.$teach['id'].'" data-table="users" title="Sub-admin"><i class="fa fa-user"></i></a>
                            </p>';
                    $statusDrop = '';
                    if($teach['status'] == 1){
                        $statusDrop = '<div class="admin_tbl_status_wrap"><a class="tbl_status_btn edu_btn_width light_sky_bg changeStatusButton" data-id="'.$teach['id'].'" data-table ="users" data-status ="0" href="javascript:;">'.$this->lang->line('ltr_active').'</a></div>';
                    }else{
                        $statusDrop = '<div class="admin_tbl_status_wrap">
                    <a class="tbl_status_btn light_red_bg edu_btn_width changeStatusButton" data-id="'.$teach['id'].'" data-table ="users" data-status ="1" href="javascript:;">'.$this->lang->line('ltr_inactive').'</a></div>';
                    }
                    if (!empty($teach['teach_image'])){ 
    ...
    function getdatateacher(){
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            $post = $this->input->post(NULL,TRUE);
            $teacher_data = current($this->db_model->select_data('*','users use index (id)',array('id'=>$this->input->post('id'))));
            // print_r($teacher_data);die;
            // $output = array()
             $data = array(
                    'data'=>$teacher_data,
                    'subject_id'=>json_decode($teacher_data['teach_subject'])
                );
     ...
     function paper_manage_edit($id=0){
    		$subject = $this->db_model->select_data('id,subject_name,no_of_questions','subjects use index (id)',array('admin_id'=>$this->session->userdata('uid'),'status'=>1),'',array('id','desc'));
    		$batch = $this->db_model->select_data('id,batch_name','batches  use index (id)',array('admin_id'=>$this->session->userdata('uid')),'',array('id','desc'));
    		$exam_paper = $this->db_model->select_data('*','exams  use index (id)',array('id'=>$_POST['id']),'',array('id','desc'));
    	    $q_old = [];
    	    $q_id = json_decode($exam_paper[0]['question_ids']);
    	    if($_SESSION['role']==3){
    	        $adId = $_SESSION['admin_id'];
    	    }else{
    	        $adId = $exam_paper[0]['admin_id'];
    	    }
	       foreach($q_id as $value){
                $data = $this->db_model->select_data('question,id','questions  use index (id)',array('admin_id'=>$adId,'id'=>$value));
                array_push($q_old,$data);
    	    }
    		if($id>0){
    			$single_question = $this->db_model->select_data('*','questions',array('added_by'=>$this->session->userdata('uid'),'id'=>$_POST['id']))[0];
    		}
    		
    // 		print_r($q_old);die;
    		
    		echo json_encode(array('batch'=>$batch,'subject'=>$subject,'question_data'=>$q_old,'exam_paper'=>$exam_paper,'single_question'=>$single_question));
	...
	function update_paper(){
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
	       ...
	       if(isset($data_arr['CreateQuestion'])==1){
	            $option1 = ($data_arr['option1']);
                $option2 = ($data_arr['option2']); 
                $option3 = ($data_arr['option3']);
                $option4 = ($data_arr['option4']);
                $data_arr1['subject_id'] = $data_arr['subject_id'];
                $data_arr1['chapter_id'] = $data_arr['chapter_id'];
                $data_arr1['question'] = $data_arr['question'];
                $data_arr1['options'] = json_encode(array($this->input->post('option1'),$this->input->post('option2'),$this->input->post('option3'),$this->input->post('option4')),JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_UNESCAPED_UNICODE);
	            $data_arr1['answer'] = $data_arr['answer'];
	            $data_arr1['question_mask'] = $data_arr['marks'];
	            if($data_arr1['answer']==""){
	               $resp = array('status'=>2,'msg'=>'Please Select Answer');
                   ...
                   // $data_arr = $this->security->xss_clean($data_arr);
                    ...
                    $ins = $this->db_model->insert_data_question('questions',$data_arr1);
                    if($ins==true){
                        $this->db_model->update_with_increment('chapters','no_of_questions',array('id'=>$this->input->post('chapter_id',TRUE)),'plus',1);
                        $this->db_model->update_with_increment('subjects','no_of_questions',array('id'=>$this->input->post('subject_id',TRUE)),'plus',1);
                        // $resp = array('status'=>1,'msg'=>$this->lang->line('ltr_question_added_msg'),'url'=>base_url($profile.'/question-manage'));
	           ...
	           $last_question_add_id = $this->db_model->select_data('id','questions  use index (id)','','1',array('id','desc'));
	           $old_question = $data_arr['old_queston']; 
               $add_old_question = $data_arr['old_question_id']; 
              
                if(isset($data_arr['CreateQuestion']) && !isset($data_arr['addQuestion']) ){
                    $last_id = array('0'=>$last_question_add_id[0]['id']);
                    $final_qquestion_id = array_merge($add_old_question,$last_id); 
                    $total_question =  count(array_merge_recursive($add_old_question,$last_question_add_id[0]));
                }else if(!isset($data_arr['CreateQuestion']) && isset($data_arr['addQuestion']) ){
                    $final_qquestion_id = array_merge($old_question,$add_old_question);
                    $total_question =  count(array_merge_recursive($add_old_question,$old_question));
                }else if(!isset($data_arr['CreateQuestion']) && !isset($data_arr['addQuestion']) ){
                    $final_qquestion_id = $add_old_question;
                    $total_question =  count($add_old_question);
                }else if(isset($data_arr['CreateQuestion']) && isset($data_arr['addQuestion']) ){
                    $question_old = array_merge($old_question,$add_old_question);
                    $last_id = array('0'=>$last_question_add_id[0]['id']);
                    $final_qquestion_id = array_merge($question_old,$last_id); 
                    $total_question = count(array_merge_recursive($add_old_question,$last_question_add_id[0],$old_question));
                }

                if($this->session->userdata('role')==1){
                   if($_POST['modifyType']=='newpaper'){
                        $data_arr2['admin_id'] = $this->session->userdata('uid');
                   }
                }else{
                   if($_POST['modifyType']=='newpaper'){
                        $data_arr2['admin_id'] = $this->session->userdata('uid');
                ...
                $data_arr2['name'] = $data_arr['name'];
                $data_arr2['type'] = $data_arr['type'];
                $data_arr2['format'] = 1;
                $data_arr2['batch_id'] = $data_arr['batch_id'];
                $data_arr2['total_question'] =  $total_question;
                $data_arr2['time_duration'] = $data_arr['time_duration'];
                $data_arr2['question_ids'] = json_encode($final_qquestion_id);
                if($this->input->post('type',TRUE) == 1){
                    $data_arr2['mock_sheduled_date'] = date('Y-m-d',strtotime($data_arr['mock_sheduled_date']));
                    $data_arr2['mock_sheduled_time'] = date('H:i:s',strtotime($data_arr['mock_sheduled_time']));
                    
                    $cond = array('type'=>$this->input->post('type',TRUE),'mock_sheduled_date'=>$data_arr['mock_sheduled_date'],'mock_sheduled_time'=>$data_arr['mock_sheduled_time'],'batch_id'=>$this->input->post('batch_id',TRUE),'admin_id'=>$admin_id,'status'=>1);
    
                    $prevData = $this->db_model->select_data('id','exams use index (id)',$cond,1);
                }else{
                    $data_arr2['mock_sheduled_date'] = '';
                    $data_arr2['mock_sheduled_time'] = '';
                    $prevData = array();
                }
                ...
                $tmk ="";
                foreach($final_qquestion_id as $mk){
                    $marks = $this->db_model->select_data('id,question_mask','questions use index (id)',array('id'=>$mk))[0];
                    $tmk +=$marks['question_mask'];
                }
                $data_arr2['total_marks'] = $tmk;
                if($this->session->userdata('role') == 1){
                    $profile = 'admin';
                }else{
                    $profile = 'teacher';
                ...
                if(empty($prevData)){
                    ...
                    if($_POST['modifyType']=='newpaper'){
                        $ins = $this->db_model->insert_data('exams',$data_arr2);               
                    }else{
                        
                        $ins = $this->db_model->update_data_limit('exams',$data_arr2,array('id'=>$_POST['expid'],'admin_id'=>$admin_id),1);
                    }
                    if($ins==true){
                        $resp = array('status'=>1,'msg'=>$this->lang->line('ltr_paper_added_msg'),'url'=>base_url($profile.'/exam-manage'));
	...
	function add_mask(){
	     if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            ...
            for($i=0;$i<count($data);$i++){
                ...
                //     print_r($mask);
            	   // die();
                    $res = $this->db_model->update_data_limit($table,array($column=>$mask),array('id'=>$id),'1');
                    if($res){
                    ...
                    // $resp = array('status'=>1,'msg'=>$this->lang->line('ltr_data_delete_msg'));
                      ...
                      $this->db_model->update_data_limit($table,array($column=>$mask),array('id'=>$id),'1');
                    $resp = array('status'=>1,'msg'=>$this->lang->line('ltr_data_delete_msg'));
                }
            }
            echo json_encode($resp,JSON_UNESCAPED_SLASHES);
    ...
    function testing(){
    	$this->load->view('admin/view_certificate1');
